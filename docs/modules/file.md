...
- download_file url [output]
  ...
  - Returns: 0 on success; the downloader's non-zero status on failure, with the output file removed so no partial or error-page file is left at the destination; 1 when neither `curl` nor `wget` is installed.
  - Args:
    - url — source URL.
...
- verify_checksum iso_file checksum_file [checksum_type=sha256sum]
  ...
  - Behavior: Finds the entry for `iso_file`'s basename in `checksum_file` (`HASH  name`, `HASH *name`, or tagged `ALGO (name) = HASH`; a listed path matches on its basename), hashes `iso_file` with `checksum_type`, and compares (case-insensitive). Other entries in the list do not matter.
